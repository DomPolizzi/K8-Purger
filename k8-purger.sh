...
rm -rf ~/.kube
rm -rf /etc/cni /etc/kubernetes /var/lib/dockershim /var/lib/etcd /var/lib/kubelet /var/lib/etcd2/ /var/run/kubernetes ~/.kube/* /usr/local/bin/kubeadm \
    /usr/local/bin/kubeadm /usr/local/bin/etcd /usr/local/bin/kubelet /usr/local/bin/kubectl \
    /etc/ssl/etcd
